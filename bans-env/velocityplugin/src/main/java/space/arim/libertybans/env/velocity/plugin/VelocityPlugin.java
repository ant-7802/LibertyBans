...
public final class VelocityPlugin {
    ...
    public synchronized void onProxyShutdown(ProxyShutdownEvent evt) {
        ...
        if (base == null) {
            logger.warn(
                "LibertyBans wasn't launched; check your log for a startup error"
            );
            return;
        }
        base.shutdown();
    ...
    private BaseFoundation initialize() {
        ...
        LibertyBansLauncher launcher = new LibertyBansLauncher.Builder()
            ...
            .platform(
                Platforms.velocity(PluginContainer.class.getClassLoader())
            )
        ...
        try {
            base = new Instantiator(
                "space.arim.libertybans.env.velocity.VelocityLauncher",
                launchLoader
            ).invoke(
